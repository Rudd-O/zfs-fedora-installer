...
import os
import shutil
import subprocess  # pylint: disable=unused-import  # noqa: F401
import tempfile
import unittest
        ...
        self.assertRaises(
            ...
            lambda: f(
                [
                    "bash",
                    "-c",
                    "echo You probably have corrupted RPMDB, running 'rpm --rebuilddb' might fix the issue.; false",
                ]
            ),
        )
        self.assertRaises(
                ...
                with mock.patch.object(cmd, "get_output_exitcode", fun):
                    ...
                    with mock.patch.object(os.path, "exists", lambda _: True):
                        with mock.patch.object(os.path, "isfile", lambda _: True):
                            c = pm.ChrootPackageManager(tmpd, "27", None)
            ...
            with mock.patch.object(cmd, "check_call_silent", fun):
                with mock.patch.object(cmd, "get_output_exitcode", fun):
                    ...
                    with mock.patch.object(os.path, "exists", exists):
                        ...
                        c = pm.ChrootPackageManager(tmpd, release, None)
